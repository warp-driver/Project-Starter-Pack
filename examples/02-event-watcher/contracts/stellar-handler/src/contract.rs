...
use message_board::MessageBoardClient;

use crate::storage;

/// Payload carried inside an `XlmEnvelope.payload`. The circuit emits
/// the matching shape as a hand-built `ScMap` with two entries keyed
/// `"msg"` and `"msg_id"`; Soroban serialises this struct as an
/// `ScMap` sorted by alphabetical field name, so `msg` precedes
/// `msg_id` in the encoded bytes — the field declaration order below
/// is irrelevant for wire format, but kept alphabetical too for
/// readability. Add fields here only by adding them to the circuit's
/// map at the same time, keeping the keys sorted.
#[contracttype]
#[derive(Clone)]
...
#[contractimpl]
impl StellarHandler {
    /// Pin both the verification contract and the downstream
    /// message-board at deploy time. The version string lets tooling
    /// confirm what's live on-chain without trusting a deploy log.
    pub fn __constructor(
        env: Env,
        verification_contract: Address,
        message_board_contract: Address,
    ) {
        storage::set_verification_contract(&env, &verification_contract);
        storage::set_message_board_contract(&env, &message_board_contract);
    ...
    ///      envelopes).
    ///   4. Decode the inner RecordPayload and relay to
    ///      message_board.record_signed.
    ...
    pub fn verify_xlm(
        ...
        let payload = RecordPayload::from_xdr(&env, &envelope.payload)
            .map_err(|_| HandlerError::InvalidEnvelope)?;

        // Cross-contract call: message_board.record_signed gates on
        // `handler.require_auth()`, which Soroban grants implicitly
        // because *this* contract is the immediate caller. No explicit
        // auth payload needed. A downstream panic (e.g. AlreadyRecorded
        // from a buggy circuit that emitted duplicate msg_ids) unwinds
        // the whole tx — the operator retries, hits EventAlreadySeen
        // on the next pass, and gives up cleanly. In the normal path
        // the handler's own seen-set above already screened duplicates.
        let mb_addr = storage::get_message_board_contract(&env);
        MessageBoardClient::new(&env, &mb_addr).record_signed(&payload.msg_id, &payload.msg);

        // Order matters: mark seen + extend TTL + emit Verified happen
        // only after the downstream call returns. A panic in
        // message-board would unwind the whole transaction, so we'd
        // never have marked a failed event as processed.
    ...
    pub fn message_board_contract(env: Env) -> Address {
        storage::get_message_board_contract(&env)
    }
