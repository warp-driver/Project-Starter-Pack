...
pub enum DataKey {
    ...
    /// Address of the downstream `message-board` contract we relay to
    /// once the signature check passes.
...
pub fn set_message_board_contract(env: &Env, addr: &Address) {
    env.storage()
        .instance()
        .set(&DataKey::MessageBoardContract, addr);
}

pub fn get_message_board_contract(env: &Env) -> Address {
    env.storage()
        .instance()
        .get(&DataKey::MessageBoardContract)
        .expect("message-board contract not set")
}
